...
- provide your username and password in the login prompt
- provide a token as the username and leave the password empty
- provide a token in the `Authorization` header

```sh
curl https://example-app.localhost --header "Authorization: Bearer my-secret-token"
```

This is just the beginning of built-in authentication mechanism in smallweb. Basic Auth is not ideal (especially on iOS), so I'm looking toward adding support for forward auth / SSO in the future (maybe through an `auth.<your-domain>` service).
...
You can also access the cli from the `cli.<your-domain>` service, either from your browser or using curl.
